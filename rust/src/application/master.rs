...
struct CommunicationActor {
    id: i32,
    context: Option<Context>,
    univserve: mpi::environment::Universe
}
impl CommunicationActor {
    fn new() -> CommunicationActor {
        CommunicationActor {
            context: None,
            id: 0,
            univserve: mpi::initialize().unwrap()
        }
    }
}
impl Actor for CommunicationActor {
    fn handle(&mut self, message: String) {
        ...
        if vec[0] == "broardcast" {
            for x in 0..self.univserve.world().size() {
                let v = vec![1,2,3];
                self.univserve.world().process_at_rank(x).send(&v[..]);
            }
        }
    ...
    fn receive_context(&mut self, context: Context) {
        ...
        println!("Spawning Master {}..", self.id);
        let ctx: &Context = self.context.as_ref().expect("");
        ctx.register_actor(MasterActor::new());
        ...
        let actor = Arc::clone(& Arc::new(self));

        tokio::spawn(lazy(move || {

            let com_actor = Arc::try_unwrap(actor).ok().unwrap();


            // let (msg, status) = universe.world()
